  ...
  const handleSaveVideo = async (formData) => {
    try {
      ...
      if (formData.videoFile) {
        payload.append("video", formData.videoFile); // ไฟล์จากเครื่อง
      } else {
        payload.append("video_url", formData.video_url); // URL พิมพ์เอง
      }
      ...
      <div className="bg-white p-5 rounded-xl border border-[var(--color-border)] shadow-sm mb-6">
        <div className="relative flex items-center">
          ...
          <input type="text" placeholder="ค้นหาชื่อสื่อมัลติมีเดีย หรือ ลิ้งก์ URL..."
            ...
            className="w-full pl-11 pr-4 py-2 border border-[var(--color-border)] rounded-xl text-base focus:outline-none focus:border-[var(--color-border-focus)] focus:ring-2 focus:ring-[var(--color-green-light)]/50 transition-all bg-[var(--color-surface)]/20 placeholder:text-[var(--color-placeholder)]"
          />
        </div>
      </div>
