...
export default function ReportRegion() {
  ...
  useEffect(() => {
    ...
    api.get("/reports/geographies")
      .then((res) => { if (res.data.success) setRegionOptions(res.data.data); })
      .catch(console.error);

    fetchData();
  }, []);

  const fetchData = async (type_id = "", geography_id = "") => {
    setLoading(true);
    try {
      const res = await api.get("/reports", {
        params: { type_id: type_id || undefined, geography_id: geography_id || undefined },
      });
  ...
  const handleProcess = (e) => {
    e.preventDefault();
    setCurrentPage(1);
    fetchData(selectedType, selectedRegion);
    Swal.fire({
  ...
  const handleExportExcel = () => {
    const exportData = filteredData.map((item, index) => ({
      ...
      "ภาค": item.geography_name || "ไม่ระบุ",
    }));
    ...
    <div className="p-6 bg-[var(--color-surface)] min-h-screen font-sans antialiased text-[var(--color-deep-text)] print:p-0 print:bg-white">
      ...
      <div className="bg-white p-5 rounded-xl border border-[var(--color-border)] shadow-sm mb-5 print-hidden">
        <form onSubmit={handleProcess} className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm font-bold text-[var(--color-deep-text)]">ภาค *:</label>
