...
// ── ข้อมูลจำลองในตาราง ถอดแบบตามโครงสร้างภาพหน้าจอของคุณ ──
const initialReportData = [
  { id: 1, name: "โครงการพัฒนาคลังความรู้การนำปรัชญาของเศรษฐกิจพอเพียงไปประยุกต์ใช้ ในรูปแบบเว็บไซต์", projectType: "โครงการภายใต้สถาบันเศรษฐกิจพอเพียง", region: "ภาคกลาง", typeId: "CAT-001", regId: "REG-CENTRAL" },
  { id: 2, name: "การบริหารจัดการเครือข่ายเศรษฐกิจพอเพียงแบบบูรณาการ", projectType: "โครงการพัฒนาแบบ บูรณาการ และ อื่นๆ", region: "ภาคกลาง", typeId: "CAT-002", regId: "REG-CENTRAL" },
  { id: 3, name: "โครงการจัดทำ Sufficiency Living Case Book ระยะที่ 1", projectType: "โครงการสวัสดิการสังคม การศึกษา", region: "ภาคกลาง", typeId: "CAT-003", regId: "REG-CENTRAL" },
  { id: 4, name: "การบริหารราชการแผ่นดินบนพื้นฐานหลักปรัชญาของเศรษฐกิจพอเพียง ระยะที่ 1", projectType: "โครงการภายใต้สถาบันเศรษฐกิจพอเพียง", region: "ภาคกลาง", typeId: "CAT-001", regId: "REG-CENTRAL" },
  { id: 5, name: "การสำรวจการรับรู้และความเข้าใจของประชาชนเกี่ยวกับปรัชญาของเศรษฐกิจพอเพียง", projectType: "โครงการภายใต้สถาบันเศรษฐกิจพอเพียง", region: "ภาคกลาง", typeId: "CAT-001", regId: "REG-CENTRAL" },
  { id: 6, name: "โครงการพัฒนาดิจิทัลแพลตฟอร์มศูนย์กลางการเรียนรู้บูรณาการปรัชญาของเศรษฐกิจพอเพียงกับนวัตกรรมเพื่อการพัฒนาที่ยั่งยืน สมดุล และมั่นคง", projectType: "โครงการภายใต้สถาบันเศรษฐกิจพอเพียง", region: "ไม่ระบุ", typeId: "CAT-001", regId: "REG-NONE" },
  { id: 7, name: "โครงการพัฒนาดิจิทัลแพลตฟอร์มศูนย์กลางการเรียนรู้บูรณาการปรัชญาของเศรษฐกิจพอเพียงกับนวัตกรรมเพื่อการพัฒนาที่ยั่งยืน สมดุล และมั่นคง", projectType: "โครงการภายใต้สถาบันเศรษฐกิจพอเพียง", region: "ไม่ระบุ", typeId: "CAT-001", regId: "REG-NONE" },
  { id: 8, name: "โครงการจัดทำ Sufficiency Living Case Book (ระยะที่ 2)", projectType: "โครงการภายใต้สถาบันเศรษฐกิจพอเพียง", region: "ไม่ระบุ", typeId: "CAT-001", regId: "REG-NONE" },
  { id: 9, name: "โครงการเชื่อมโยงองค์ความรู้ด้านเศรษฐกิจพอเพียงเพื่อความยั่งยืนแบบบูรณาการ", projectType: "โครงการภายใต้สถาบันเศรษฐกิจพอเพียง", region: "ไม่ระบุ", typeId: "CAT-001", regId: "REG-NONE" },
  { id: 10, name: "โครงการวิจัยการบริหารราชการแผ่นดินบนพื้นฐานหลักปรัชญาของเศรษฐกิจพอเพียง ระยะที่ 2", projectType: "โครงการภายใต้สถาบันเศรษฐกิจพอเพียง", region: "ไม่ระบุ", typeId: "CAT-001", regId: "REG-NONE" }
];
...
export default function ReportRegion() {
  ...
  const [processedRegion, setProcessedRegion] = useState("");
  const [searchTerm, setSearchTerm] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  // ฟังก์ชันกดยืนยันการประมวลผลข้อมูลรายงาน
  const handleProcess = (e) => {
    e.preventDefault();
    setProcessedType(selectedType);
    setProcessedRegion(selectedRegion);
    setCurrentPage(1);
    Swal.fire({
      title: "ประมวลผลสำเร็จ",
      text: "ระบบคัดกรองข้อมูลตามประเภทและพื้นที่ภาคเรียบร้อยแล้ว",
      icon: "success",
      timer: 1200,
  ...
  // 🛠️ อัปเดตรอบนี้เป็นระบบดาวน์โหลดไฟล์ .xlsx แท้ผ่านไลบรารี SheetJS
  const handleExportExcel = () => {
    // 1. ปรับโครงสร้างข้อมูล Array ให้หัวคอลัมน์เป็นภาษาไทยตรงตามตารางหน้า ReportRegion
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "แยกตามภาค");

    // 3. สั่งเขียนไฟล์และเด้งหน้าต่างดาวน์โหลดเป็น .xlsx ออกมาทันที
    XLSX.writeFile(workbook, `รายงานแยกตามภาค_${new Date().toISOString().slice(0, 10)}.xlsx`);

    // แจ้งเตือนความสำเร็จสวยๆ ด้วย SweetAlert2
    Swal.fire({
  ...
  // การกรองข้อมูลด้วยเครื่องมือค้นหา และค่าตัวเลือกตัวกรองหลังกดประมวลผล
  const filteredData = data.filter((item) => {
    const matchesType = processedType === "" || item.typeId === processedType;
    const matchesRegion = processedRegion === "" || item.regId === processedRegion;
  ...
  return (
    <div className="p-6 bg-[var(--color-surface)] min-h-screen font-sans antialiased text-[var(--color-deep-text)] print:p-0 print:bg-white">
      ...
      {/* ── CSS ควบคุมมิติจัดหน้าตัดเลื่อน Scrollbar และแก้โครงสร้างลีบแบนตอนสั่งพิมพ์ ── */}
      <style>{`
        @media print {
          ...
          /* 🛠️ กำจัด Scrollbar ออกอย่างถาวรในหน้ากระดาษ */
      ...
      {/* ── PRINT HEADER (พ่นหัวเอกสารเฉพาะตอนสั่งพิมพ์สไตล์คลาสสิก) ── */}
      ...
      {/* ── ตัวกรองคัดกรองคู่ Dropdown ประเภทโครงการ และ ภาค ── */}
      <div className="bg-white p-5 rounded-xl border border-[var(--color-border)] shadow-sm mb-5 print-hidden">
        <form onSubmit={handleProcess} className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          ...
          <div className="flex flex-col gap-1">
            ...
            >
              <option value="">-เลือกทั้งหมด-</option>
              <option value="REG-CENTRAL">ภาคกลาง</option>
              <option value="REG-NONE">ไม่ระบุ</option>
            </select>
          </div>
      ...
      {/* ── ACTIONS BAR BUTTONS & SEARCH INPUT ── */}
