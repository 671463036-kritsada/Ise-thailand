            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div>
                    <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-[var(--color-forest-green)] tracking-tight flex items-center gap-2">
                        <CalendarDays className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-[var(--color-green)] shrink-0" />
                        <span>{typeName || 'กำลังโหลด...'}</span>
                    </h1>
                    <p className="text-xs sm:text-sm lg:text-base text-[var(--color-muted-text)] mt-0.5 font-medium">
                        จัดการข้อมูล {typeName || 'กิจกรรม'} ทั้งหมดในระบบ
                    </p>
                </div>
                <button
                    onClick={() => { setEditData(null); setModalOpen(true); }}
                    className="w-full sm:w-auto px-5 py-2.5 bg-[var(--color-green)] text-white text-sm sm:text-base font-bold rounded-xl shadow-md hover:bg-[var(--color-forest-green)] transition-all duration-200 flex items-center justify-center gap-1.5 cursor-pointer shrink-0 active:scale-95"
                >
                    <Plus className="w-5 h-5 stroke-[2.5]" />
                    <span>เพิ่ม{typeName || 'กิจกรรม'}</span>
                </button>
            </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[50rem]">
                        <thead>
                            <tr className="bg-[var(--color-surface-2)] border-b border-[var(--color-surface-3)] text-[var(--color-forest-green)] text-xs sm:text-sm lg:text-base">
                                ...
                                <th className="px-4 sm:px-5 py-3 font-bold min-w-[16rem]">ชื่อกิจกรรม</th>
                                <th className="px-4 sm:px-5 py-3 font-bold w-44 whitespace-nowrap">วันที่จัดกิจกรรม</th>
                                <th className="px-4 sm:px-5 py-3 font-bold w-28 text-center whitespace-nowrap">ไฟล์ PDF</th>
                                <th className="px-4 sm:px-5 py-3 font-bold w-32 text-center whitespace-nowrap">จัดการ</th>
                            </tr>
                        </thead>
                                ...
                                currentItems.map((row, idx) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 sm:px-5 py-3.5 font-semibold text-[var(--color-deep-text)]">
                                            <div className="line-clamp-2 leading-relaxed">{row.title}</div>
                                            {row.title_eng && (
                                        ...
                                        <td className="px-4 sm:px-5 py-3.5 text-[var(--color-muted-text)] font-semibold whitespace-nowrap">
                                            {row.activity_date
