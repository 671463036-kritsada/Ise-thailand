...
import PlanAllModel from "../model/PlanAllModel"; // นำเข้าโมดอลจัดการแผนงาน

const initialPlans = [
  {
    id: "PLAN-001",
    name: "แผนงานวิจัยและนวัตกรรมเพื่อการพัฒนาพื้นที่และลดความเหลื่อมล้ำ"
  },
  {
    id: "PLAN-002",
    name: "แผนงานส่งเสริมการเพิ่มขีดความสามารถในการแข่งขันของอุตสาหกรรมชุมชน"
  },
  {
    id: "PLAN-003",
    name: "แผนงานอนุรักษ์และฟื้นฟูทรัพยากรธรรมชาติและสิ่งแวดล้อมอย่างมีส่วนร่วม"
  },
  {
    id: "PLAN-004",
    name: "แผนงานพัฒนาศักยภาพผู้ประกอบการรายย่อยและวิสาหกิจชุมชนยุคดิจิทัล"
  }
];
...
export default function PlanAll() {
  ...
  const currentItems = filteredPlans.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  const handleSavePlan = (formData) => {
    if (modalMode === "delete") {
      ...
      setPlans((prev) => prev.map((item) => item.id === formData.id ? formData : item));
  ...
  return (
    <div className="p-6 bg-[var(--color-surface)] min-h-screen font-sans antialiased text-[var(--color-deep-text)]">
      ...
      {/* ── DATA TABLE (สเกลฟอนต์ระดับกลาง text-base ตามฟิลด์ที่กำหนด) ── */}
      <div className="bg-white rounded-xl border border-[var(--color-border)] shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-3 border-t border-[var(--color-surface-3)] flex flex-col sm:flex-row justify-between items-center gap-3 bg-[var(--color-surface)]/10 text-xs">
          <p className="font-medium text-[var(--color-muted-text)]">
            แสดง {filteredPlans.length > 0 ? currentItems.length : 0} จากทั้งหมด {filteredPlans.length} รายการ
          </p>
      ...
      {/* โมดอลจัดการฟอร์ม เพิ่ม/แก้ไข/ลบ แผนงาน */}
      {isModalOpen && (
        <PlanAllModel 
          isOpen={isModalOpen}
